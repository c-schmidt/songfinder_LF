...
class proxy:
    ...
    def storebookmark(self, station):
        ...
        f = open(os.path.join(self.basedir, "ws-auth.txt"), "w")
        for s in self.bookmarks:
            f.write(s + "\n")
        f.close()
    ...
    def resolveBindAddress(self):
        bind_address = config.bind_address
        if bind_address.find(".") <0 :
	    tupel = socket.getaddrinfo(bind_address,1881)
	    bind_address = tupel[0][4][0]
	    print "address "+config.bind_address+" resolved as "+bind_address
	return bind_address
	...
	sys.stderr.flush()
        ...
        #print "*** http host is "+http["Host"]
        #print "*** req is "+str(req)
	#print "*** http "+str(http)
	#peerIp = clientsock.getpeername()[0]
	...
	if station == "/remoteRadio":
	    print "remoteRadio called"
	    
	    #file = "C:\\Programme\\Winamp\\winamp.exe http://localhost:"+str(config.listenport)+"/lastfm.mp3"
	    #file = "C:\\Programme\\Winamp\\winamp.exe C:\\Dokume~1\\gaga\\Lokale~1\\Temp\\lastfm.m3u"
	    #file = "C:\\Programme\\Winamp\\winamp.exe http://localhost:"+str(config.listenport)+"/lastfm.m3u"
	    #file="C:\Dokume~1\gaga\Lokale~1\Temp\lastfm.m3u"
	    file="C:\\Programme\\lastfmproxy-1.4c\\data\\lastfm.m3u"
	    
	    
	    
	    
	    ##webbrowser.open_new("http://localhost:"+str(config.listenport)+"/lastfm.m3u")
	    ...
	    #
	    proc = subprocess.Popen([ file, ""],                        creationflags=DETACHED_PROCESS, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
	    pid = proc.pid
	    ...
	    cont="<html><head><meta HTTP-EQUIV='REFRESH' content='0; url=http://" + self.resolveBindAddress() + ":" + str(config.listenport) + "/'></head><body><p>OK, remote radio started.</p><a href='http://" + self.resolveBindAddress() + ":" + str(config.listenport) + "/"+"'>Tune in </a></body></html>\n"
	    print "cont is "+cont
            try:
                clientsock.sendall("HTTP/1.0 200 OK\r\n")
                clientsock.sendall(self.extraheaders)
	...
	if station == "/callback":
	    print "received callback, authentication OK. Start the radio, please."
	    #print req
	    startIdx  =  len("/callback?token=")
	    token = req[0][1][startIdx:]
	    if self.lastfm.debug:
	       print "token is "+token
	       # redirect to out start page
	    cont="<html><head><meta HTTP-EQUIV='REFRESH' content='0; url=http://" + self.resolveBindAddress() + ":" + str(config.listenport) + "/'></head><body><p>OK, received token "+token+".</p><a href='http://" + self.resolveBindAddress() + ":" + str(config.listenport) + "/"+"'>Tune in </a></body></html>\n"
	    ...
	    #print "try to get ws_session"
	    self.lastfm.wsSession(token)
	    sys.stdout.flush()
	    sys.stderr.flush()
	    return
        ...
        if station == "/":
            ...
            tmp = "LastFMProxy v" + self.version + " - &copy; 2005-2007 <a href=\"http://vidar.gimp.org/lastfmproxy/\" target=\"_blank\">Vidar Madsen</a> 2010 continued by <a href=\"http://www.gabifrings.de/download.html\" target=\"_blank\">Gabi Frings</a>"
	    ...
	    peerIp = clientsock.getpeername()[0]
	    bind_address = self.resolveBindAddress()
	    print "peerIp " +peerIp + " bind_address "+bind_address
	    
	    if  peerIp != bind_address :
               tmp = tmp + "<a href=\"http://" + http["Host"] + "/remoteRadio\">Start remote radio</a>\n"
	    else:
	...
	print "bind_address is "+bind_address
...
p.run(p.resolveBindAddress(), config.listenport)
